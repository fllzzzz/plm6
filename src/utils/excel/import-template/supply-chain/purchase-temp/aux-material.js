...
const sectionSteelSpecTmpl = {
  ...
  fields: [
    ...
    { label: '颜色', field: 'color', excelField: '__EMPTY_7' },
  ...
  format: async (tableList) => {
    ...
    // 获取 钢板科目
    const matList = store.state.config.rawMatClsList.filter((v) => v.basicClass === matClsEnum.MATERIAL.V)
    ...
    for (const row of list) {
      ...
      const specification = row.specification ?? '' // 对应采购“规格”列
