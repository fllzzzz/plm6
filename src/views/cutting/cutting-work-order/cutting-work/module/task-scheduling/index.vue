      ...
      <div class="el-drawer-container">
        <div class="table-content">
          ...
          >
            ...
            <!-- <el-table-column align="center" key="source" prop="source" :show-overflow-tooltip="true" label="名称" min-width="30">
              <template v-slot="scope">
            ...
            <el-table-column key="weight" prop="weight" :show-overflow-tooltip="true" label="单重（kg）" min-width="40" align="center">
              <template v-slot="scope">
                <span>{{ scope.row.weight }}</span>
              </template>
            </el-table-column>
            <el-table-column key="weight" prop="weight" :show-overflow-tooltip="true" label="总重（kg）" min-width="40" align="center">
              <template v-slot="scope">
                <span>{{ scope.row.weight }}</span>
              </template>
            </el-table-column>
...
async function plateDataGet() {
  ...
  try {
    // const data = await get({ projectId: props.detailData.projectId, pageSize: page.size, pageNumber: page.page,plateID:props.detailData.id })
    const data = await get({ projectId: props.detailData.projectId, pageSize: page.size, pageNumber: page.page,plateID:props.detailData.id })
    page.total = data.totalElements
    plateData.value = data.content.filter(item => !(item.mac))
     
    // plateData.value = data.content.filter(item => item)
