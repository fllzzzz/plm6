      ...
      <el-form ref="formRef" size="small" :model="form" :rules="rules" label-width="110px">
        <div style="display:flex;" class="detail-header">
            ...
            <div :class="showType === 'detail'?'flex-div':''">
              ...
              <div :class="showType === 'detail'?'child-div':''">
                ...
                <el-form-item label="运输单价">
                  <span style="margin-right:3px;">{{ toFixed(detailInfo.supplier.price, decimalPrecision.mes) }}</span>
                ...
                <el-form-item label="运输费">
                  <span><span style="margin-right:3px;">{{ detailInfo.totalPrice?toFixed(detailInfo.totalPrice, decimalPrecision.mes):'-'}}</span>元</span>
                </el-form-item>
              </div>
            </div>
          </div>
          <div style="flex:1;padding-left:8px;" v-if="showType === 'edit'">
            ...
            <el-form-item label="运输费" v-if="form.priceType === logisticsPriceTypeEnum.WEIGHT.V">
              <span><span style="margin-right:3px;">{{ toFixed(allPrice, decimalPrecision.mes) }}</span>元</span>
            </el-form-item>
        ...
        >
          ...
          <el-table-column prop="price" label="运输费（元）" align="right">
            <template v-slot="scope">
              <span style="margin-right:3px;">{{ toFixed(scope.row.totalPrice, decimalPrecision.mes) }}</span>
            </template>
          </el-table-column>
