  ...
  >
    ...
    <template #content>
      ...
      >
        ...
        <el-table-column :show-overflow-tooltip="true" prop="groups.name" label="生产组" align="center">
          <template #default="{ row }">
            <span>{{ row.workshop?.name }}>{{ row.groups?.name }}</span>
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" align="center" prop="completeTime" label="生产日期" width="120">
          <template #default="{ row }">
...
const { visible: productionDrawerVisible, handleClose } = useVisible({ emit, props, field: 'visible', showHook: fetchList })
...
const queryParams = computed(() => {
  return {
    projectId: props.productionInfo.project?.id
  }
})
