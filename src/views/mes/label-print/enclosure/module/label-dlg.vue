<template>
  ...
  >
    <table border="1" bordercolor="#000000">
      ...
      <tr>
        <td style="font-size: 25pt; font-weight: bold" colspan="3">{{ component.name }}-{{ component.serialNumber }}</td>
      </tr>
      <tr>
        <td>颜色： {{ component.color }}</td>
        <td colspan="2">规格： {{ component.plateType }}</td>
      </tr>
      <tr>
        <td>厚度(mm): {{ component.thickness }}</td>
        <td>长度(mm): {{ component.length }}</td>
        <td>数量(张)：{{ component.quantity }}</td>
      </tr>
      <tr>
