<template>
  <common-table :data="tableData" :max-height="maxHeight" style="width: 100%">
    ...
    <el-table-column key="plate" prop="plate" show-overflow-tooltip label="板型">
      <template v-slot="scope">
        <span v-empty-text>{{ scope.row.plate }}</span>
      </template>
    </el-table-column>
    <el-table-column key="color" prop="color" show-overflow-tooltip label="颜色">
      <template v-slot="scope">
        <span v-empty-text>{{ scope.row.color }}</span>
      </template>
    </el-table-column>
    ...
    <el-table-column key="width" prop="width" show-overflow-tooltip label="宽度(mm)" align="center">
      <template v-slot="scope">
        <span v-empty-text>{{ toFixed(scope.row.width, DP.MES_ENCLOSURE_W__MM) }}</span>
      </template>
    </el-table-column>
    ...
    <el-table-column key="totalArea" prop="totalArea" show-overflow-tooltip label="总面积(㎡)" align="center">
      <template v-slot="scope">
        <span v-empty-text>{{ convertUnits(scope.row.totalArea, 'mm2','m2', DP.COM_AREA__M2) }}</span>
      </template>
    </el-table-column>
  </common-table>
</template>
