    ...
    <crudOperation :show-grid="false" :show-refresh="false">
      ...
      <template #viewLeft>
        <span v-permission="crud.permission.get">
          ...
          <el-tag effect="plain" class="filter-item">
            <span>出库量：</span>
            <span v-if="!summaryLoading">{{ toFixed(summaryInfo.outboundMete, DP.MES_ENCLOSURE_L__M) }} m</span>
            <i v-else class="el-icon-loading" />
          </el-tag>
...
const defaultQuery = {
  ...
  plate: '',
  color: '',
  type: mesWarehouseStateTypeEnum.OUTBOUND.V,
...
CRUD.HOOK.handleRefresh = (crud, res) => {
  res.data.content = res.data.content.map((v) => {
    ...
    v.boxColor = getColor(v, { quantity: 'outboundQuantity', compare: 'compareQuantity' })
    return v
  })
}
