<template>
  <common-drawer ref="drawerRef" title="制成品记录" v-model="taskDrawerVisible" direction="rtl" :before-close="handleClose" size="70%">
    <template #titleAfter>
      <el-tag size="small" effect="plain">
        项目：<span>{{ upsInfo.project?.serialNumber }}-{{ upsInfo.project?.name }}</span>
      </el-tag>
    </template>
    <template #titleRight>
      <print-table
        v-permission="permission.print"
        api-key="mesUpsStatisticsList"
        :params="{ ...queryParams }"
        size="mini"
        type="warning"
        class="filter-item"
...
async function fetchList() {
  try {
    ...
    const { content = [], totalElements } = await getUps({
      ...queryParams.value,
      ...queryPage
    })
