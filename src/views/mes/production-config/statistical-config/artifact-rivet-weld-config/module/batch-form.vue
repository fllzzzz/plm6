    ...
    <el-form ref="formRef" :model="form" :disabled="submitLoading">
      ...
      >
        ...
        <el-table-column prop="specPrefixStr" :show-overflow-tooltip="true" label="截面前缀" width="150" align="center" fixed="left">
          <template #default="{ row, $index }">
            <span v-if="row.classificationId === -1 && $index !== 0">同上</span>
...
const rivetWeldListObj = inject('rivetWeldListObj')
...
// 同上的选项与值
const ditto = new Map([['classificationId', -1]])
const rowNumbers = 5
...
const tableProcessObj = computed(() => {
  ...
  let curClassificationId = null
  for (let i = 0; i < form.list.length; i++) {
    curClassificationId = form.list[i].classificationId !== -1 ? form.list[i].classificationId : curClassificationId
    _obj[i] = curClassificationId
  }
  return _obj
})
...
function handleClassificationChange(val, index) {
  let _processObj = {}
  if (isNotBlank(artifactTypeListObj.value[val]?.typeProcessObj)) {
    _processObj = artifactTypeListObj.value[val]?.typeProcessObj
  }
