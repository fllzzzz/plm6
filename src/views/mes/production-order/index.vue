<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column v-if="columns.visible('project')" key="project" prop="project" label="所属项目" align="center" min-width="150" :show-overflow-tooltip="true" />
      ...
      <el-table-column v-if="columns.visible('quantity')" key="quantity" prop="quantity" label="清单量（件/吨）" align="center" :show-overflow-tooltip="true" min-width="150">
        <template v-slot="scope">
          <span v-if="scope.row.quantity && scope.row.netWeight">{{`${scope.row.quantity} | ${scope.row.netWeight}`}}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="columns.visible('status')" key="status" prop="status" label="状态" align="center">
        <template v-slot="scope">
          <el-tag :type="scope.row.status===scheduleStatusEnum.NOT.V?'warning':(scope.row.status===scheduleStatusEnum.COMPLETED.V?'success':'')">{{ scheduleStatusEnum.VL[scope.row.status] }}</el-tag>
        </template>
      </el-table-column>
      <!--编辑与删除-->
      <el-table-column
      ...
      >
        <template v-slot="scope">
          ...
          <common-button type="primary" size="mini" @click="openDetail(scope.row, 'audit')">计划排期</common-button>
        </template>
      </el-table-column>
    </common-table>
