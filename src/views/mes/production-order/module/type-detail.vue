  ...
  <div class="detail-container">
    <div style="margin-bottom:10px;min-height:28px;" class="tag-div">
      <div style="float:left;">
        <el-tag v-if="props.currentRow.classificationId === null">{{`${componentTypeEnum.VL[productType]}分类:`}}未知类型</el-tag>
        ...
        <template v-if="productType === componentTypeEnum.MACHINE_PART.V ">
          ...
          <template v-else>
            <el-input v-model.trim="specification" type="text" placeholder="请输入规格" style="width: 200px;margin:0 10px;" @blur="fetchDetail"/>
          </template>
        </template>
      </div>
...
import { artifactDetail, assembleDetail, machinePartDetail } from '@/api/mes/production-order-manage/production-order'

import useMaxHeight from '@compos/use-max-height'
import { DP } from '@/settings/config'
import { toThousand } from '@/utils/data-type/number'
import { isNotBlank } from '@data-type/index'
import { componentTypeEnum, hasHoleEnum } from '@enum-ms/mes'
...
const params = computed(() => {
  return { projectId: props.currentRow.projectId, material: props.currentRow.material, classificationName: props.currentRow.classificationName }
})
...
async function fetchDetail() {
  ...
  const query = props.productType === componentTypeEnum.MACHINE_PART.V ? {
    ...params.value,
    boolHaveHole: boolHaveHole.value,
    specification: specification.value
  } : { ...params.value }
  try {
    const api = props.productType === componentTypeEnum.ARTIFACT.V ? artifactDetail : (props.productType === componentTypeEnum.ASSEMBLE.V) ? assembleDetail : machinePartDetail
