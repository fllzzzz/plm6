...
const originOption = [
  ...
  { label: '压型板(t)', key: 'contourPlate', dateKey: 'contourPlateDate', no: TechnologyTypeAllEnum.ENUM.PROFILEDPLATE.V, alias: 'ENCLOSURE' },
  { label: '桁架楼承板(t)', key: 'trussFloorPlate', dateKey: 'trussFloorPlateDate', no: TechnologyTypeAllEnum.ENUM.TRUSSFLOORPLATE.V, alias: 'ENCLOSURE' },
  { label: '夹芯板(t)', key: 'battenBoard', dateKey: 'battenBoardDate', no: TechnologyTypeAllEnum.ENUM.SANDWICH_BOARD.V, alias: 'ENCLOSURE' },
  { label: '压型楼承板(t)', key: 'pressureBearingPlate', dateKey: 'pressureBearingPlateDate', no: TechnologyTypeAllEnum.ENUM.PRESSUREBEARINGPLATE.V, alias: 'ENCLOSURE' }
]
...
const rules = {
  ...
  pressureBearingPlate: [
    { required: true, message: '请填写', trigger: 'blur', type: 'number' }
  ],
  pressureBearingPlateDate: [
    { validator: checkOtherDate, trigger: 'change' }
  ],
  subStructure: [
    { required: true, message: '请填写', trigger: 'blur', type: 'number' }
  ...
  (val) => {
    if (isNotBlank(val)) {
      ...
      val.projectContentList.forEach(v => {
        if (val.businessType === businessTypeEnum.ENUM.MACHINING.V) {
          if (v.no && originOption.findIndex(k => k.no === v.no) > -1) {
            ...
            // this.$set(currentOption.value, currentOption.value.length - 1, optionVal)
          }
      ...
      if (currentOption.value.findIndex(k => k.alias === 'ENCLOSURE') > -1) {
        const optionVal = { label: '折边件(t)', key: 'flangingPiece', dateKey: 'flangingPieceDate', alias: 'ENCLOSURE' }
        currentOption.value.push(optionVal)
        // this.$set(currentOption.value, currentOption.value.length - 1, optionVal)
      }
    }
