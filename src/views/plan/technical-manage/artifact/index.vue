<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column v-if="columns.visible('surfaceArea')" key="surfaceArea" prop="surfaceArea" sortable="custom" :label="`面积\n(㎡)`" align="left" min-width="80px">
        <template v-slot="scope">
      ...
      <el-table-column v-if="columns.visible('createTime')" key="createTime" prop="createTime" label="上传时间" min-width="160px">
        <template v-slot="scope">
          <div><span v-parse-time="'{y}-{m}-{d}'">{{ scope.row.createTime }}</span></div>
        </template>
      </el-table-column>
    </common-table>
      <!--分页组件-->
