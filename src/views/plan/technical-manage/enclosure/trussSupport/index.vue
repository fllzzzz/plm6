<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column v-if="columns.visible('serialNumber')" key="serialNumber" prop="serialNumber" sortable="custom" :show-overflow-tooltip="true" label="编号" min-width="100px">
      ...
      <el-table-column v-if="columns.visible('width')" key="material" prop="material" sortable="custom" :show-overflow-tooltip="true" :label="`有效宽度\n(mm)`" min-width="120px">
        <template v-slot="scope">
          <template v-if="scope.row.mainEdit">
            <el-input-number
      ...
      <el-table-column v-if="columns.visible('quantity')" key="quantity" prop="quantity" sortable="custom" label="数量(张)" align="left" min-width="120px">
        <template v-slot="scope">
          <template v-if="scope.row.mainEdit">
            <el-input-number
      ...
      <el-table-column v-if="columns.visible('totalArea')" key="netWeight" prop="netWeight" sortable="custom" :show-overflow-tooltip="true" :label="`总面积`" align="left" min-width="80px">
        <template v-slot="scope">
          {{ scope.row.totalArea? scope.row.totalArea.toFixed(DP.COM_AREA__M2): '-' }}
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('totalLength')" key="totalLength" prop="totalLength" sortable="custom" :label="`总长度(m)`" align="left" min-width="80px">
        <template v-slot="scope">
...
// crud交由presenter持有
const permission = {
  get: ['trussSupport:get'],
  edit: ['trussSupport:edit'],
  del: ['trussSupport:del'],
  editStatus: ['trussSupport:editStatus'],
  importList: ['trussSupport:import'],
}
