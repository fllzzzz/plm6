<template>
  <div class="app-container">
    <template v-if="globalProject">
      ...
      >
        ...
        <el-table-column v-if="columns.visible('stockMete')" key="stockMete" prop="stockMete" :show-overflow-tooltip="true" label="当前库存(kg)" min-width="160px">
          <template v-slot="scope">
            <span style="cursor: pointer;" @click="stockDetail=true;">{{ scope.row.stockMete }}</span>
          </template>
        </el-table-column>
    ...
    <steelStockDetail v-model="stockDetail" />
  </div>
</template>
...
import steelStockDetail from './module/steel-stock-detail'
