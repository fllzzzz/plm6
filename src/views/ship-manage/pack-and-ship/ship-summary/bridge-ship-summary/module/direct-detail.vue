<template>
  <div v-show="!props.showType && props.category === mesShipStatisticsTypeEnum.DIRECT.V" class="my-code" style="margin-top: 20px">
    *点击上方表格数据查看详情
  </div>
  <div v-show="props.showType" style="margin-top: 20px">
    <div class="head-container" v-show="props.category === mesShipStatisticsTypeEnum.DIRECT.V">
      ...
      <div class="filter-item" style="float: right">
        ...
        v-permission="permission.print"
          :api-key="
            showType === 'INVENTORY'
              ? 'bridgeShipMeteDetailDirect'
              : showType === 'ASSIGNMENT'
              ? 'bridgeShipTaskMeteDetailDirect'
              : showType === 'STORAGE'
              ? 'bridgeShipInboundMeteDetailDirect'
              : showType === 'CUMULATIVE_SHIPMENT'
              ? 'bridgeShipTotalMeteDetailDirect'
              : showType === 'SHIPMENT_MONTH'
              ? 'bridgeShipMonthMeteDetailDirect'
              : showType === 'IN_STOCK'
              ? 'bridgeShipStockMeteDetailDirect'
              : 'bridgeShipTrainMeteDetailDirect'
          "
          :params="{
    ...
    <common-table
      v-show="props.category === mesShipStatisticsTypeEnum.DIRECT.V"
      :data="list"
...
import { summaryDetailDirect } from '@/api/ship-manage/pack-and-ship/bridge-ship-summary'
...
async function fetchDetail() {
  ...
  try {
    const { content = [], totalElements } = await summaryDetailDirect({
      projectId: props.projectId,
      ...props.query,
      workshopId: props.workshopId,
