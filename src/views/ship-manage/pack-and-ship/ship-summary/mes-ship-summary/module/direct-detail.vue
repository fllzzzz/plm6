    ...
    <div class="head-container" v-show="props.category === mesShipStatisticsTypeEnum.DIRECT.V">
      ...
      <div class="filter-item" style="float: right">
        <print-table
          :api-key="
            showType === 'INVENTORY'
              ? 'mesShipMeteDetailDirect'
              : showType === 'ASSIGNMENT'
              ? 'mesShipTaskMeteDetailDirect'
              : showType === 'STORAGE'
              ? 'mesShipInboundMeteDetailDirect'
              : showType === 'CUMULATIVE_SHIPMENT'
              ? 'mesShipTotalMeteDetailDirect'
              : showType === 'SHIPMENT_MONTH'
              ? 'mesShipMonthMeteDetailDirect'
              : showType === 'IN_STOCK'
              ? 'mesShipStockMeteDetailDirect'
              : 'mesShipTrainMeteDetailDirect'
          "
          :params="{
...
import { structureDirectSummaryDetail } from '@/api/ship-manage/pack-and-ship/ship-summary'
...
async function fetchDetail() {
  ...
  try {
    const { content = [], totalElements } = await structureDirectSummaryDetail({
      projectId: props.projectId,
      ...props.query,
      workshopId: props.workshopId,
