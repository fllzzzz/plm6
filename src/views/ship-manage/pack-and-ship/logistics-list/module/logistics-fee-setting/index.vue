  ...
  <common-drawer ref="drawerRef" title="物流费设置" v-model="drawerVisible" direction="rtl" :before-close="handleClose" size="50%">
    ...
    <template #content>
      ...
      >
          ...
          <template v-slot="scope">
            <div style="width: 100%">
              <span :class="scope.row.priceType === logisticsPriceTypeEnum.WEIGHT.V ? 'blue' : 'orange'">
                <span>{{ toFixed(scope.row.price, decimalPrecision.shipment) }}</span>
        ...
        >
          <template v-slot="scope">
            <span v-if="scope.row.tax">{{ toFixed(scope.row.tax, 2) }}%</span>
...
<script setup>
import { logisticsPrice as crudApi } from '@/api/ship-manage/pack-and-ship/logistics-list'
import { getCarModelConfig } from '@/api/config/mes/base'
import { getSupplierCarPrice } from '@/api/ship-manage/pack-and-ship/logistics-list'
