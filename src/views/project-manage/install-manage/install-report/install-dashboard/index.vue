        ...
        <template v-for="item in boardList" :key="item.id">
          ...
          >
            <div class="board-box" :style="{ 'background-color': `${item.boxColor}`, ...boxStyle }">
              ...
              <span class="ellipsis-text">{{ item.installQuantity }}/{{ item.quantity }}</span>
            </div>
          </el-tooltip>
        </template>
...
<script setup>
import { installDashboardData as get } from '@/api/project-manage/install-manage/install-report-list'
import { ref, watch } from 'vue'

import { businessTypeEnum } from '@enum-ms/contract'
import { installReportDashboardPM as permission } from '@/page-permission/project'
