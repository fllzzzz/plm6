<template>
  <div class="app-container">
    ...
    >
      ...
      />
      <el-table-column
        v-if="columns.visible('boxSpecList')"
        key="boxSpecList"
        prop="boxSpecList"
        label="规格前缀"
        :show-overflow-tooltip="true"
      ...
      >
        <template v-slot="scope">
          <template v-if="scope.row.boxSpecList && scope.row.boxSpecList.length > 0">
            <span v-for="item in scope.row.boxSpecList" :key="item.id">
              {{ `【${item.specPrefix}】`}}
            </span>
          </template>
        </template>
