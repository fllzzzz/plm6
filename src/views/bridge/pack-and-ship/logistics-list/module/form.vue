      ...
      <el-form ref="formRef" size="small" :model="form" :rules="rules" label-width="110px">
            ...
            <div :class="showType === 'detail'?'flex-div':''">
              ...
              <div :class="showType === 'detail'?'child-div':''">
                ...
                <el-form-item label="运输单价">
                  <span style="margin-right:3px;">{{ toFixed(detailInfo.supplier.price, decimalPrecision.bridge) }}</span>
                ...
                <el-form-item label="运输费">
                  <span><span style="margin-right:3px;">{{ detailInfo.totalPrice?toFixed(detailInfo.totalPrice, decimalPrecision.bridge):'-'}}</span>元</span>
                </el-form-item>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <el-table-column prop="price" label="运输单价" align="right">
            <template v-slot="scope">
              <span style="margin-right:3px;">{{ toFixed(scope.row.price, decimalPrecision.bridge) }}</span>
          ...
          <el-table-column prop="price" label="运输费（元）" align="right">
            <template v-slot="scope">
              <span style="margin-right:3px;">{{ toFixed(scope.row.totalPrice, decimalPrecision.bridge) }}</span>
            </template>
          </el-table-column>
