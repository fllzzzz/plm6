  ...
  <div class="app-container">
    <template v-if="pageShow">
      ...
      >
        ...
        <el-table-column v-if="columns.visible('serialNumber')" prop="serialNumber" :show-overflow-tooltip="true" align="center" label="分段编号">
          <template #header>
...
<script setup>
import crudApi, { boxCellDetail } from '@/api/bridge/bridge-plan/box-list'
...
async function load({ row, treeNode, resolve }) {
  try {
    const { content } = await boxCellDetail({ boxId: row.id })
    let childIndex = 1
    if (content.length > 0) {
      content.map((v) => {
        v.dataType = 1
