    ...
    >
      <el-expand-table-column :data="crud.data" v-model:expand-row-keys="expandRowKeys" row-key="id" fixed="left">
        <template #default="{ row: { sourceRow: row } }">
          ...
          <p>
            可生产产品种类<span>{{ row.typeSequence }}</span>
          </p>
          <p>
...
const dataFormat = ref([['targetProductionShow', ['to-fixed', 2]]])
...
CRUD.HOOK.handleRefresh = (crud, { data }) => {
  data.content.forEach((v) => {
    ...
    v.linkIdList = v.type ? getBitwiseBack(v.type) : []
    v.typeSequence = v.linkIdList?.map((v) => `【${TechnologyTypeAllEnum.VL?.[v]}】`)?.join('')
  })
}
