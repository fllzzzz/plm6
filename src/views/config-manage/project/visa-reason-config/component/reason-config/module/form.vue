    ...
    <template #content>
      <el-form ref="formRef" :model="form" size="small" label-width="140px">
        ...
        >
          ...
          <el-table-column prop="name" label="签证原因" align="center" min-width="270">
            <template v-slot="scope">
              <el-input v-model="scope.row.name" type="text" placeholder="签证原因" style="width: 260px" maxlength="50"/>
            </template>
          </el-table-column>
...
const tableRules = {
  name: [{ required: true, message: '请输入签证原因', trigger: 'blur' }]
}
...
CRUD.HOOK.beforeValidateCU = (crud, form) => {
  if (crud.form.list.length <= 0) {
    ElMessage({ message: '请先填写签证原因明细', type: 'error' })
    return false
  }
  const rules = tableRules
  let flag = true
