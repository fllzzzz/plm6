...
<el-card shadow="always">
    <template #header>
      <div class="clearfix">
        <span class="card-title">分包签证原因</span>
    ...
    >
      ...
      <el-table-column v-if="columns.visible('name')" key="name" prop="name" :show-overflow-tooltip="true" label="分包签证原因" min-width="150">
        <template v-slot="scope">
          <el-input v-model="scope.row.name" type="text" placeholder="分包签证原因" style="width: 260px" maxlength="50" v-if="scope.row.edit"/>
...
<script setup>
import crudApi from '@/api/config/project-config/visa-reason-config'
import { ref } from 'vue'

import { projectVisaReasonConfigPM as permission } from '@/page-permission/config'
