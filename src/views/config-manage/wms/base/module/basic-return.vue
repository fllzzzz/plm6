<template>
  <el-card shadow="always">
    <template #header>
      <div class="clearfix">
        <span class="card-title">退库审核配置</span>
        <common-tip-button
          v-permission="permission.basicReturnEdit"
    ...
    <el-form v-loading="dataLoading" :disabled="formDisabled" :model="form" label-position="left" label-width="130px">
      <el-form-item label="审核方式">
        <common-radio v-model="form.returnType" :options="returnAuditEnum.ENUM" type="enum" size="small" />
      </el-form-item>
      <el-form-item label-width="0">
        <span class="form-item-tip">办理退库时选择通过方式，默认提交需要审核，可配置提交即通过</span>
        <span class="form-item-tip">审核后通过: 物料退库提交后需审核通过则视为退库成功:</span>
        <span class="form-item-tip">提交即通过:物料退库无需审核处理，提交后即视为退库成功:</span>
      </el-form-item>
    </el-form>
  </el-card>
</template>
...
<script setup>
import { getReturnBasicConf, setReturnBasicConf } from '@/api/config/wms/base'
...
async function fetchData() {
  ...
  try {
    const res = await getReturnBasicConf()
...
async function submit() {
  try {
    ...
    await setReturnBasicConf(form.value)
    ElNotification({
      title: '退库审核配置设置成功',
      type: 'success',
      duration: 2500
