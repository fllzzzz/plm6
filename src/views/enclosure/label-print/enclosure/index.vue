<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template v-slot="scope">
          <table-cell-tag :show="scope.row.boolPrinted" name="已打印" color="#e64242" :offset="15" />
    ...
    <!--分页组件-->
    <pagination />
    <printed-record-drawer v-model:visible="recordVisible" :task-id="currentTaskId" :getEnclosureRecord="getEnclosureRecord" />
    <label-dlg v-model:visible="labelVisible" :label-data="currentLabel" :productType="productType" :labelType="labelType" />
  </div>
</template>

<script setup>
import { getEnclosureRecord } from '@/api/mes/label-print/enclosure'
...
const { crud, columns, CRUD } = useCRUD(
  {
    ...
    optShow: { ...optShow },
    crudApi: { ...crudApi },
    requiredQuery: ['planId', 'productionLineId'],
...
CRUD.HOOK.beforeToQuery = () => {
  crud.query.lineId = crud.query.productionLineId
}
