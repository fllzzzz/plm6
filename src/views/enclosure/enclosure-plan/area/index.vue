<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column v-if="columns.visible('signingDate')" key="signingDate" prop="signingDate" label="合同签订时间" align="center" width="150px">
        <template v-slot="scope">
          <span>{{ scope.row.signingDate?parseTime(scope.row.signingDate,'{y}-{m}-{d}'):'-' }}</span>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('signerName')" key="signerName" prop="signerName" :show-overflow-tooltip="true" align="center" label="销售负责人" width="120px">
        <template v-slot="scope">
          <span>{{ scope.row.signerName || '-' }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="columns.visible('createName')" key="createName" prop="createName" :show-overflow-tooltip="true" label="创建人" width="120px" />
