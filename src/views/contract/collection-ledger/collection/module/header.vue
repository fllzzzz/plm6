<template>
  <div>
    <div v-show="crud.searchToggle">
      <project-radio-button :disabled="query.type === 2" size="small" :type="'all'" v-model="query.projectId" class="filter-item" @change="crud.toQuery" />
      <common-radio-button :options="projectOrScrapEnum.ENUM" type="enum" v-model="query.type" class="filter-item" @change="crud.toQuery" showOptionAll />
      <el-date-picker
    ...
    <crudOperation>
      <template #optLeft>
        <print-table
          ...
          class="filter-item"
        />
        <el-tag class="filter-item" size="medium"><span>收款合计：</span>{{ totalNumber }}</el-tag>
      </template>
    </crudOperation>
  </div>
</template>
...
import { getTotalNumber } from '@/api/contract/collection-and-invoice/collection'
