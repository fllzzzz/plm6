...
import { audit } from '@/api/contract/supplier-manage/jd-product-logistics-payment'
...
const totalFreight = computed(() => {
  const freightData = list.value?.map((v) => v.totalPrice)
...
async function fetchList() {
  ...
  try {
    const { content = [] } = await logisticsIsPaymentList({ supplierId: props.detailInfo.supplierId, projectId: props.detailInfo.projectId, supplierPaymentId: props.currentRow.id })
