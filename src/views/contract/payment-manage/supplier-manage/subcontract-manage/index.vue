  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="paymentRate" label="付款比例" align="center" show-overflow-tooltip min-width="80">
       ...
       <el-table-column prop="payable" label="应付账款" align="right" show-overflow-tooltip min-width="120">
        <template v-slot="scope">
          <span :style="`color:${(scope.row.sourceRow.amount-scope.row.sourceRow.paymentAmount)<0?'red':''}`">{{toThousand(scope.row.sourceRow.amount-scope.row.sourceRow.paymentAmount)}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="invoiceAble" label="应收发票" align="right" show-overflow-tooltip min-width="120">
        <template v-slot="scope">
          <span :style="`color:${(scope.row.sourceRow.amount-scope.row.sourceRow.invoiceAmount)<0?'red':''}`">{{toThousand(scope.row.sourceRow.amount-scope.row.sourceRow.invoiceAmount)}}</span>
        </template>
      </el-table-column>
      <!--付款和收票-->
      <el-table-column
    ...
    >
      <template #titleAfter>
        <span>分包单位：{{detailInfo.supplierName}}</span>
      </template>
      <template #content>
