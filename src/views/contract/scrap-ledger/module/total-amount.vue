<template>
  <common-drawer v-model="visible" :before-close="handleClose" size="100%" title="累计出售额">
    <template #titleRight>
      <print-table api-key="totalScrapDetaile" :params="{ branchCompanyId: props.currentRow.branchCompanyId, contractWasteId: props.currentRow.contractWasteId }" />
    </template>
    <template #content>
...
async function fetchData() {
  try {
    const { content } = await totalAmountList({ branchCompanyId: props.currentRow.branchCompanyId, contractWasteId: props.currentRow.contractWasteId })
    console.log(content)
