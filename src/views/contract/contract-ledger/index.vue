<template>
  ...
  >
    ...
    <el-table-column v-if="columns.visible('collectionAmount')" key="collectionAmount" prop="collectionAmount" label="累计收款" align="center" min-width="130px">
      <template v-slot="scope">
        <el-tag @click="openTab(scope.row.id,'collection')" effect="plain">{{ isNotBlank(scope.row.collectionAmount)? toThousand(scope.row.collectionAmount): '-' }}</el-tag>
      </template>
    </el-table-column>
    <el-table-column v-if="columns.visible('collectionRate')" key="collectionRate" prop="collectionRate" label="收款比例" align="center" min-width="100px">
      <template v-slot="scope">
        <div>{{ scope.row.collectionRate? scope.row.collectionRate*100+'%': '-' }}</div>
      </template>
    </el-table-column>
    <el-table-column v-if="columns.visible('invoiceAmount')" key="invoiceAmount" prop="invoiceAmount" label="累计开票" align="center" min-width="130px">
      <template v-slot="scope">
    ...
    <el-table-column v-if="columns.visible('invoiceRate')" key="invoiceRate" prop="invoiceRate" label="收款比例" align="center" min-width="100px">
      <template v-slot="scope">
        <div>{{ scope.row.invoiceRate? scope.row.invoiceRate*100+'%': '-' }}</div>
      </template>
    </el-table-column>
    <el-table-column v-if="columns.visible('deliverInstallAmount')" key="deliverInstallAmount" prop="deliverInstallAmount" label="累计发生额" align="center" min-width="130px">
      <template v-slot="scope">
        <el-tag @click="openOccurAmount(scope.row.id)" effect="plain">{{ isNotBlank(scope.row.deliverInstallAmount)? toThousand(scope.row.deliverInstallAmount): '-' }}</el-tag>
      </template>
    </el-table-column>
    <el-table-column v-if="columns.visible('availableBalance')" key="availableBalance:" prop="availableBalance:" label="可用余额" align="center" min-width="130px">
      <template v-slot="scope">
        <div>{{ scope.row.availableBalance? toThousand(scope.row.availableBalance): '-' }}</div>
      </template>
    </el-table-column>
    <el-table-column v-if="columns.visible('status')" key="status" prop="status" label="项目状态" align="center" width="90px">
      <template v-slot="scope">
