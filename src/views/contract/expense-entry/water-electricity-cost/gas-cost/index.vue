...
import crudApi, { getGasTree } from '@/api/contract/expense-entry/gas-cost'
...
async function gasListGet() {
  try {
    gasTree.value = (await getGasTree({ basicClassEnum: matClsEnum.GAS.V })) || []
...
// 默认选中第一个末级
function setRowDetail(tree = []) {
  if (tree.length) {
    if (tree[0].children?.length) {
      setRowDetail(tree[0].children)
    } else {
      nodeClick(tree[0])
    }
  }
