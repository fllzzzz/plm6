      ...
      <el-form  v-loading="crud.detailLoading" ref="formRef" size="mini" label-width="100px" class="demo-form">
        ...
        <div class="rule-row">
          ...
          <el-form-item label="合同额" prop="contractAmount">
            <div><span v-thousand="detail?.project?.contractAmount" />（{{ digitUppercase(detail?.project?.contractAmount || 0) }}）</div>
          </el-form-item>
        </div>
        <div class="rule-row">
        ...
        <div class="rule-row">
          <el-form-item label="加工结算额" prop="processingSettlementAmount">
            <div><span v-thousand="detail.processingSettlementAmount" />（{{ digitUppercase(detail.processingSettlementAmount) }}）</div>
          </el-form-item>
...
<script setup>
import { check, downloadSettlement } from '@/api/contract/sales-manage/visa-change'
...
const detailTitle = computed(() => {
  return props.status === visaReviewStatusEnum.UNREVIEWED.V ? '结算单待确签' : '结算单详情'
})
