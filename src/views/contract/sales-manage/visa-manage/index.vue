    ...
    >
      ...
      <el-table-column v-if="columns.visible('project')" key="project" prop="project" :show-overflow-tooltip="true" label="项目" min-width="250">
        <template #default="{ row }">
          <table-cell-tag :show="row.isSubmitSettle === settlementStatusEnum.SETTLED.V" :name="settlementStatusEnum.SETTLED.L" color="#f56c6c"/>
          <span style="margin-left: 14px">{{ row.project }}</span>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('reasonName')" key="reasonName" prop="reasonName" :show-overflow-tooltip="true" align="center" label="签证原因" min-width="110" />
      <el-table-column v-if="columns.visible('amount')" prop="amount" key="amount" label="签证额" align="right" min-width="120" show-overflow-tooltip />
      ...
      <el-table-column v-permission="[...permission.detail, ...permission.edit, ...permission.audit]" label="操作" width="140" fixed="right">
        <template #default="{ row }">
          ...
          <common-button v-else-if="!row.settlementRecord" v-permission="permission.edit" type="primary" size="mini" @click.stop="crud.toEdit(row)">编辑</common-button>
