  ...
  >
    ...
    <el-table-column prop="thickness" :show-overflow-tooltip="true" align="center" label="板厚(mm)">
     ...
     <el-table-column prop="coating" :show-overflow-tooltip="true" align="center" label="涂层">
      <template v-slot="scope">
        <span :style="`color:${!isNotBlank(scope.row.originVal)?scope.row.color:''};`">
          <template v-if="isNotBlank(scope.row.originVal) && !judgeSameValue(scope.row.coating,scope.row.originVal.coating)">
            <cell-change-preview :old="scope.row.originVal.coating" :new="scope.row.coating" />
          </template>
          <template v-else>
    ...
    <el-table-column prop="colour" :show-overflow-tooltip="true" align="center" label="颜色">
      <template v-slot="scope">
         <span :style="`color:${!isNotBlank(scope.row.originVal)?scope.row.color:''};`">
          <template v-if="isNotBlank(scope.row.originVal) && !judgeSameValue(scope.row.colour,scope.row.originVal.colour)">
            <cell-change-preview :old="scope.row.originVal.colour" :new="scope.row.colour" />
          </template>
          <template v-else>
...
const tableArr = computed(() => {
  const arr = []
  console.log(props.tableData, props.originData)
