  ...
  <div class="production-lines-content">
    <template v-if="!selectedAbleLineLoading">
      <template v-if="selectedAbleLineIds && selectedAbleLineIds.length > 0">
        <template v-for="line in enclosureProductLines" :key="line.id">
          <el-tooltip
            class="item"
            effect="light"
            :content="`${line.name}`"
...
import useEnclosureProductLines from '@compos/store/use-enclosure-product-lines'
...
const { loaded, enclosureProductLines } = useEnclosureProductLines()
...
function getLines() {
  ...
  return enclosureProductLines.value
}
