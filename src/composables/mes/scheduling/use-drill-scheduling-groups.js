...
import { getDrillGroupsTree } from '@/api/mes/scheduling-manage/artifact'

// 获取排产使用的生产班组信息：工厂-车间-生产线-生产班组
const useDrillSchedulingGroups = ({ queryParams, factoryIds, disabledIds }) => {
  ...
  async function fetchGroupsTree() {
    ...
    const content = await getDrillGroupsTree(queryParams.value)
...
export async function manualFetchGroupsTree({ productType, structureClassId, _factoryIds, disabledIds = [] }) {
  ...
  const content = await getDrillGroupsTree({ productType, structureClassId })
  // 零件返回全部
...
export default useDrillSchedulingGroups
